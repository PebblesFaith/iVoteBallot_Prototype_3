function toggle_InputPasswordCheckBox() {
	var passwordInput = document.getElementById("temporary_Password");
...
const validateHRMsVerifyTempPasswordForm = (e) => { 	
	...
	if (EmployeeTemporary_Password === '' || EmployeeTemporary_Password == null || EmployeeTemporary_Password.length <= 7 || !EmployeeTemporary_Password.match(regExPassword)) {
		document.getElementById('userEmployeeTemporary_PasswordErrorMessage').textContent = 'Your temporary password is a required input field.';
	...
	form.addEventListener('submit', validateHRMsVerifyTempPasswordForm);
