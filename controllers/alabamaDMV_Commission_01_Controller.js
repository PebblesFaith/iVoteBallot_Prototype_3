...
passport.use(
	...
	async (req, userDMVEmail, userCommissionIvoteBallotIdCodeBcryptic, done) => {
		...
		console.log('The user have manually entered his or her email (\'userDMVEmail\') to which, passes through the passport.use(login3) to the session cookie id as: ' + userDMVEmail);
		console.log('The user have manually entered his or her password (\'userCommissionIvoteBallotIdIdentifierCode\') to which, passes through the user\'s passport.use(login3) to the session cookie id as: ' + userCommissionIvoteBallotIdCodeBcryptic);
...
const redirect_SignUp_Dashboard = (req, res, next) => {
    if(req.session.userId) {
